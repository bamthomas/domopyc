...
class MysqlTemperatureMessageHandler(object):
    ...
    @asyncio.coroutine
    def handle(self, average_message):
        with (yield from self.db) as conn:
            cursor = yield from conn.cursor()
            yield from cursor.execute(
                'INSERT INTO {table_name} (timestamp, temperature) values (%s, %s)'.format(table_name=self.table_name), (
